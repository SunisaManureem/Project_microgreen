...
static bool is_in_light_window_test(time_t now_ts) {
    struct tm ti;
    localtime_r(&now_ts, &ti);
    // เปิดเมื่อวินาทีอยู่ในช่วง [6, 19] รวมปลายทั้งคู่
    return (ti.tm_sec >= TEST_LIGHT_SEC_START && ti.tm_sec <= TEST_LIGHT_SEC_END);
}
#endif
...
static void apply_light_policy_minute(int m, time_t now_ts) {
    ...
    if (m < OFF_DAYS) {
        ...
        if (is_in_light_window_test(now_ts)) {
            grow_on();
            ESP_LOGI(TAG_GROW, "Minute %d: LIGHT = ON (sec %d–%d of each minute)",
                     m + 1, TEST_LIGHT_SEC_START, TEST_LIGHT_SEC_END);
        } else {
            grow_off();
            ESP_LOGI(TAG_GROW, "Minute %d: LIGHT = OFF (outside sec %d–%d window)",
                     m + 1, TEST_LIGHT_SEC_START, TEST_LIGHT_SEC_END);
        }
    } else {
        // นาที 8+ : OFF
