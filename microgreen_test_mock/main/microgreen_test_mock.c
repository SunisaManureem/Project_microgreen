...
// + TEST MODE: 1 minute ≡ 1 day, minute 1–3 OFF, 4–7 ON (first 14s), 8+ OFF until reset) =====
#include <stdio.h>
#include <string.h>
#include <time.h>
...
/* ==================== TEST MODE (FOR EASY DEMO) ====================
   ตั้ง 1 นาที = 1 วัน: นาที 1–3 ปิด, นาที 4–7 เปิดเฉพาะ 14 วินาทีแรกของแต่ละนาที,
   นาที >=8 ปิดยาวจนกดปุ่มรีเซ็ต NVS start_ts  */
#define GROW_TEST_MINUTE_DAY    1        // 1=เปิดโหมดทดสอบ, 0=โหมดจริง
#define TEST_LIGHT_SEC_PER_MIN  14       // เปิดไฟ 14 วินาทีแรกของทุกนาทีในช่วง 4–7
/* ================================================================== */
...
/* ======== TEST helper: เปิดเฉพาะ 14 วินาทีแรกของแต่ละนาที (ช่วงนาที 4–7 เท่านั้น) ======== */
#if GROW_TEST_MINUTE_DAY
static bool is_in_light_window_test(time_t now_ts) {
    struct tm ti;
    localtime_r(&now_ts, &ti);
    return (ti.tm_sec < TEST_LIGHT_SEC_PER_MIN);
}
#endif
...
static void apply_light_policy_minute(int m, time_t now_ts) {
    ...
    if (m < OFF_DAYS) {
        grow_off();
        ESP_LOGI(TAG_GROW, "Minute %d: LIGHT = OFF (seedling dark phase)", m + 1);
    } else if (m < TOTAL_DAYS) {
        if (is_in_light_window_test(now_ts)) {
            grow_on();
            ESP_LOGI(TAG_GROW, "Minute %d: LIGHT = ON (first %ds of each minute)", m + 1, TEST_LIGHT_SEC_PER_MIN);
        } else {
            grow_off();
            ESP_LOGI(TAG_GROW, "Minute %d: LIGHT = OFF (rest of the minute)", m + 1);
        }
    } else {
        grow_off();
        ESP_LOGI(TAG_GROW, "Minute %d+: LIGHT = OFF (beyond planned 7 minutes)", m + 1);
    }
}
#endif
        ...
        if (lvl == 0) { // กำลังกด
            ...
            if (was_low && press_start_ms != 0) {
                ...
                if (held >= WIFI_CLEAR_HOLD_MS) {
                    ...
                    nvs_wifi_clear_lock();
                    nvs_clear_start_ts();
                    time(&start_ts);                 // เริ่มรอบใหม่ทันที
                    nvs_set_start_ts(start_ts);
