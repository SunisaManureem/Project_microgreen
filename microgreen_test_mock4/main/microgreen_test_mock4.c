...
/* ===== Logging cadence (ปรับให้ถี่ขึ้น) ===== */
static const uint32_t HEARTBEAT_SOIL_MS   = 2000;   // เดิม 5000 → 2000 ms
static const uint32_t HEARTBEAT_DHT_MS    = 2000;   // เดิม 5000 → 2000 ms
static const uint32_t GROW_STATUS_LOG_MS  = 60000;  // รายงานสถานะไฟปลูกทุก 1 นาที
static const int      DHT_FAIL_MAX        = 10;     // จำนวนครั้งล้มเหลวก่อนเตือนค้าง
...
/* ================= GPIO helpers ================= */
/* เก็บสถานะไฟปลูกแบบ global ให้ log ได้ตลอดเวลา */
static volatile bool g_light_on = false;

static inline void grow_on(void)  {
    gpio_set_level(GROW_RELAY_GPIO, GROW_ACTIVE_LEVEL);
    g_light_on = true;
}
static inline void grow_off(void) {
    gpio_set_level(GROW_RELAY_GPIO, GROW_INACTIVE_LEVEL);
    g_light_on = false;
}
...
static bool is_in_light_window_test(time_t now_ts) {
    (void)now_ts; // ใช้เวลาปัจจุบันจาก gettimeofday()
    struct timeval tv;
    ...
    return true; // ในช่วง 6..19 วินาที
}
#endif
...
static int units_since_start(time_t start_ts, time_t now) {
    ...
    if (now <= start_ts) return 0;
    double diff_sec = difftime(now, start_ts);
    return (int)(diff_sec / 60.0);  // 1 unit = 1 minute (TEST)
#else
...
static void apply_light_policy_minute(int m, time_t now_ts) {
    // log เมื่อสถานะไฟเปลี่ยนจริง (ลดสแปม)
...
static void grow_cycle_task(void *arg) {
    ...
    static uint32_t grow_status_ms = 0;   // สำหรับ log สถานะไฟทุก 1 นาที
    // ยิง log สถานะไฟครั้งแรกทันทีหลังบูต
    ...
    while (1) {
        ...
        /* ---- รายงานสถานะไฟปลูกทุก 1 นาที (เหมือนเดิมแต่ชัดเจนขึ้น) ---- */
        if (elapsed_since(grow_status_ms, GROW_STATUS_LOG_MS)) {
            grow_status_ms = ms32();
#if GROW_TEST_MINUTE_DAY
            bool inwin = is_in_light_window_test(now);
            ESP_LOGI(TAG_GROW, "STATUS(min): minute=%d LIGHT=%s (%s) window=%s",
                     display, g_light_on?"ON":"OFF",
                     (display<=OFF_DAYS)? "seedling dark phase" :
                        (display<=TOTAL_DAYS? "minutes 4–7 policy" : "beyond 7"),
                     inwin? "IN-SEC(6–19)" : "OUT-SEC");
#else
        ...
        if (elapsed_since(hb_ms, 5000)) { // debug heartbeat คงไว้ 5s (เฉพาะ grow)
            hb_ms = ms32();
...
static void soil_pump_task(void *arg) {
    ...
    while (1) {
        ...
        /* ---- LOG ถี่ขึ้นทุก 2 วินาที พร้อมสถานะปั๊ม ---- */
...
static void dht22_fan_task(void *arg) {
    ...
    while (1) {
        ...
        if (!ok) {
            ...
            /* ---- LOG ถี่ขึ้นทุก 2 วินาที พร้อมสถานะพัดลม ---- */
