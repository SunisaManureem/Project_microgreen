...
ปรับตามที่คุย: เปิดน้ำ -> เปิดโซลินอยด์ก่อน แล้วค่อยเปิดปั๊ม
            ปิดน้ำ -> ปิดปั๊มก่อน แล้วค่อยปิดโซลินอยด์
*/
#define RELAY_SOLENOID_GPIO     GPIO_NUM_25
#define SOLENOID_PREOPEN_DELAY_MS    200   // เปิดโซลินอยด์ก่อนปั๊ม (ms)
...
*** ปรับให้ "ขึ้นกับชนิดพืช" ตามที่อ้างอิงไว้ ***
- กลุ่ม ข้าวสาลี/ผักบุ้ง/ทานตะวัน (มาตรฐาน): ปิด 1–3, เปิด 4–7
- กลุ่ม ข้าวสาลี/ผักบุ้ง/ทานตะวัน (งานวิจัย): ปิด 1–2, เปิด 3–7 เวลา 18:00–23:00 (5 ชม.)
- คะน้า: ปิด 1–3, เปิด 4–7 แบบ 16 ชม. / ปิด 8 ชม.
- โต้วเหมี่ยว: ปิด 1–3, เปิด 4–9
หมายเหตุ: โค้ด "โหมดหน้าต่างไฟ" มีทั้ง REL (อิง start_ts) และ ABS (อิงเวลาในวัน)
*/
#define GROW_AUTO_RESET_AFTER_7 0

/* --- ค่าเริ่มต้น (fallback) ของโหมดหน้าต่างไฟแบบสัมพัทธ์จากเวลาเริ่มปลูก --- */
#define LIGHT_WINDOW_MODE_RELATIVE   1      // ใช้โหมด RELATIVE เป็นค่าเริ่มต้น
#define REL_LIGHT_OFFSET_H           6      // เริ่มเปิดหลังเริ่มปลูก 6 ชั่วโมง (default)
#define REL_LIGHT_OFFSET_MIN         0
#define REL_LIGHT_DURATION_H         14     // เปิด 14 ชั่วโมง (default)
...
/* ====== โปรไฟล์ไฟปลูก (ขึ้นกับชนิดพืช) ====== */
typedef struct {
    int  off_days;        // จำนวนวันแรกที่ "ปิดไฟ" (Day 1..off_days)
    int  total_days;      // จำนวนวันในรอบ (เช่น 7 หรือ 9)
    bool use_relative;    // true=REL window (อิง start_ts), false=ABS window (อิงเวลาในวัน)
...
/* ================== โปรไฟล์พืชตามที่อ้างอิงไว้ ==================
1) ข้าวสาลี / ผักบุ้ง / ทานตะวัน
    - Soil: ระดับปานกลาง (คงเป็นค่าช่วง % ภายในระบบ)
    - RH: 65–90%
    - Temp: < 30°C
    - Light (มาตรฐาน): ปิด 1–3, เปิด 4–7
    - Light (งานวิจัย): ปิด 1–2, เปิด 3–7 เวลา 18:00–23:00 (5 ชม.)
2) คะน้า
    - RH: 60±2%
    - Temp: < 25±2°C
    - Light: ปิด 1–3, เปิด 4–7 แบบ 16/8
3) โต้วเหมี่ยว
    - Temp: 20–25 (สูงสุด 30)
    - RH: 60–70%
    - Light: ปิด 1–3, เปิด 4–9
*/

/* ===== กลุ่ม ข้าวสาลี/ผักบุ้ง/ทานตะวัน (ค่า soil “ปานกลาง” ใช้ช่วงเดียวกัน) ===== */
...
static const crop_profile_t PROFILE_WHEAT = {
    .soil_th_low      = 60,
    .soil_th_high     = 75,
    .fan_on_temp      = 30.0f,
    .fan_off_temp     = 28.0f,
...
static const crop_profile_t PROFILE_PAKBUNG = {
    .soil_th_low      = 60,
    .soil_th_high     = 75,
    .fan_on_temp      = 30.0f,
    .fan_off_temp     = 28.0f,
...
static const crop_profile_t PROFILE_KALE = {
    .soil_th_low      = 60,
    .soil_th_high     = 75,
    .fan_on_temp      = 27.0f,   // คุมให้ไม่เกิน ~25±2
    .fan_off_temp     = 25.0f,
    .fan_on_rh        = 62.0f,   // 60±2
    .fan_off_rh       = 60.0f,
    .soil_sat_on_pct  = 83,
    .soil_sat_off_pct = 76,
};
...
/* ===== โต้วเหมี่ยว ===== */
static const crop_profile_t PROFILE_PEASHOOT = {
    .soil_th_low      = 60,
    .soil_th_high     = 75,
    .fan_on_temp      = 26.0f,   // ช่วงเหมาะ 20–25 (ยอมให้เริ่มระบายที่ ~26)
    .fan_off_temp     = 24.0f,
    .fan_on_rh        = 70.0f,   // 60–70
    .fan_off_rh       = 65.0f,
    .soil_sat_on_pct  = 83,
    .soil_sat_off_pct = 76,
};
...
/* ====== โปรไฟล์ไฟปลูก ====== */
/* มาตรฐานกลุ่ม (ปิด 3 วันแรก เปิด 4–7) ใช้ REL window (คงแนวเดิม start+6h → 14h) */
static const light_policy_t LIGHT_STD_3OFF_4TO7_REL14 = {
    .off_days       = 3,
    .total_days     = 7,
    .use_relative   = true,
    .rel_offset_h   = 6,
    .rel_offset_min = 0,
    .rel_duration_h = 14,
    .abs_start_h    = 0, .abs_start_m = 0, .abs_duration_h = 0
};
/* งานวิจัยกลุ่ม (ปิด 2 วัน เปิดวัน 3 เวลา 18:00–23:00 (5 ชม.)) ใช้ ABS window */
static const light_policy_t LIGHT_RESEARCH_2OFF_3TO7_ABS18_5H = {
    .off_days       = 2,
    .total_days     = 7,
    .use_relative   = false,
    .rel_offset_h   = 0, .rel_offset_min = 0, .rel_duration_h = 0,
    .abs_start_h    = 18,
    .abs_start_m    = 0,
    .abs_duration_h = 5
};
/* คะน้า: ปิด 3 วันแรก เปิด 4–7 แบบ 16 ชม / 8 ชม (ใช้ REL ให้ไม่ต้องผูกเวลาในวัน) */
static const light_policy_t LIGHT_KALE_3OFF_4TO7_REL16 = {
    .off_days       = 3,
    .total_days     = 7,
    .use_relative   = true,
    .rel_offset_h   = 6,
...
/* โต้วเหมี่ยว: ปิด 3 วัน เปิด 4–9 (ชั่วโมงแสงไม่ได้ระบุ → ใช้ REL 14h แบบเดิม) */
static const light_policy_t LIGHT_PEASHOOT_3OFF_4TO9_REL14 = {
    .off_days       = 3,
    .total_days     = 9,
    .use_relative   = true,
    .rel_offset_h   = 6,
...
/* โปรไฟล์ที่ใช้งานจริงตอนนี้ (ค่าเริ่มต้น sunflower ตามกลุ่ม 65–90%, <30C, ไฟปิด3เปิด4-7) */
...
/* โปรไฟล์ไฟปลูกปัจจุบัน */
static light_policy_t g_light_policy = {
    .off_days       = 3,
    .total_days     = 7,
    .use_relative   = true,
    .rel_offset_h   = 6,
...
static char g_light_policy_id[24] = "std"; // ใช้ log เฉย ๆ
...
static bool solenoidOn = false;   // <<< added
...
/* Grow-day counter (capped by policy total_days) */
...
/* ================= NVS (last_cmd_ms) =================
กันการสั่ง start_cycle_ms ซ้ำหลังรีบูต (B กันเหนียว)
*/
static esp_err_t nvs_get_last_cmd_ms(int64_t *out) {
    if (!out) return ESP_ERR_INVALID_ARG;
    nvs_handle_t nvh;
    esp_err_t err = nvs_open("grow", NVS_READONLY, &nvh);
    if (err != ESP_OK) return err;
    int64_t v = 0;
    err = nvs_get_i64(nvh, "last_cmd_ms", &v);
    nvs_close(nvh);
    if (err == ESP_OK) *out = v;
    return err;
}
static esp_err_t nvs_set_last_cmd_ms(int64_t v) {
    nvs_handle_t nvh;
    esp_err_t err = nvs_open("grow", NVS_READWRITE, &nvh);
    if (err != ESP_OK) return err;
    err = nvs_set_i64(nvh, "last_cmd_ms", v);
    if (err == ESP_OK) err = nvs_commit(nvh);
    nvs_close(nvh);
    return err;
}
...
static bool is_in_light_window_relative_params(time_t start_ts, time_t now_ts,
                                            int off_days, int total_days,
                                            int offset_h, int offset_min, int duration_h)
{
    ...
    // Day 1..off_days OFF
    if (day_idx <= off_days) return false;
    // beyond planned days OFF
    if (day_idx > total_days) return false;

    int start_s = offset_h*3600 + offset_min*60;
    int end_s   = (start_s + duration_h*3600) % 86400;
...
/* ================= Grow-Light (REAL-DAY counter + per-crop policy) ================= */
...
static void apply_light_policy_day(int d, time_t now_ts, time_t start_ts) {
    ...
    // OFF phase
    if (day_idx <= g_light_policy.off_days) {
        grow_off();
        ESP_LOGI(TAG_GROW, "Day %d: LIGHT = OFF (dark phase, policy=%s)", day_idx, g_light_policy_id);
        return;
    }

    // beyond cycle
    if (day_idx > g_light_policy.total_days) {
        grow_off();
        ESP_LOGI(TAG_GROW, "Day %d+: LIGHT = OFF (beyond planned days, policy=%s)", day_idx, g_light_policy_id);
        return;
    }

    // ON window (REL or ABS)
    bool inwin = false;
    if (g_light_policy.use_relative) {
        inwin = is_in_light_window_relative_params(start_ts, now_ts,
                                                g_light_policy.off_days, g_light_policy.total_days,
                                                g_light_policy.rel_offset_h,
                                                g_light_policy.rel_offset_min,
                                                g_light_policy.rel_duration_h);
        if (inwin) {
            grow_on();
            ESP_LOGI(TAG_GROW, "Day %d: LIGHT = ON (REL: +%dh for %dh, policy=%s)",
                    day_idx, g_light_policy.rel_offset_h, g_light_policy.rel_duration_h, g_light_policy_id);
        } else {
            grow_off();
            ESP_LOGI(TAG_GROW, "Day %d: LIGHT = OFF (REL window, policy=%s)", day_idx, g_light_policy_id);
        }
    } else {
        // ABS: เปิดตามเวลาในวัน
        inwin = is_in_light_window_absolute(now_ts,
                                            g_light_policy.abs_start_h,
                                            g_light_policy.abs_start_m,
                                            g_light_policy.abs_duration_h);
        if (inwin) {
            grow_on();
            ESP_LOGI(TAG_GROW, "Day %d: LIGHT = ON (ABS %02d:%02d for %dh, policy=%s)",
                    day_idx,
                    g_light_policy.abs_start_h, g_light_policy.abs_start_m,
                    g_light_policy.abs_duration_h, g_light_policy_id);
        } else {
            grow_off();
            ESP_LOGI(TAG_GROW, "Day %d: LIGHT = OFF (ABS window, policy=%s)", day_idx, g_light_policy_id);
        }
    }
}
...
static void set_pump(bool on)
{
    ...
    if (on) {
        /* เปิดน้ำ: เปิดโซลินอยด์ก่อน แล้วค่อยเปิดปั๊ม */
        if (!solenoidOn) {
            relay2ch_write(RELAY_SOLENOID_GPIO, true);
            solenoidOn = true;
            vTaskDelay(pdMS_TO_TICKS(SOLENOID_PREOPEN_DELAY_MS));
        }
        relay2ch_write(RELAY_PUMP_GPIO, true);
    } else {
        /* ปิดน้ำ: ปิดปั๊มก่อน แล้วค่อยปิดโซลินอยด์ */
        relay2ch_write(RELAY_PUMP_GPIO, false);
        vTaskDelay(pdMS_TO_TICKS(SOLENOID_POSTCLOSE_DELAY_MS));
        if (solenoidOn) {
            relay2ch_write(RELAY_SOLENOID_GPIO, false);
            solenoidOn = false;
        }
    }
...
/* ================= Crop profile helper =================
ค่า control/crop_profile ที่รองรับ:
- "sunflower" / "wheat" / "pakbung"          (มาตรฐาน: ปิด3 เปิด4-7)
- "sunflower_research" / "wheat_research" / "pakbung_research"  (งานวิจัย: ปิด2 เปิด3-7 18:00-23:00)
- "kale"                                     (16/8, ปิด3 เปิด4-7)
- "peashoot"                                 (ปิด3 เปิด4-9)
*/
static void apply_crop_profile_by_id(const char *id)
{
    const crop_profile_t *p = NULL;
    const light_policy_t *lp = NULL;
    const char *lp_id = "std";

    // --- กลุ่มมาตรฐาน ---
    if (strcmp(id, "sunflower") == 0) {
        p = &PROFILE_SUNFLOWER;
        lp = &LIGHT_STD_3OFF_4TO7_REL14;
        lp_id = "std_3off_4to7_rel14";
    } else if (strcmp(id, "wheat") == 0) {
        p = &PROFILE_WHEAT;
        lp = &LIGHT_STD_3OFF_4TO7_REL14;
        lp_id = "std_3off_4to7_rel14";
    } else if (strcmp(id, "pakbung") == 0) {
        p = &PROFILE_PAKBUNG;
        lp = &LIGHT_STD_3OFF_4TO7_REL14;
        lp_id = "std_3off_4to7_rel14";
    }
    // --- กลุ่มงานวิจัย (18:00–23:00) ---
    else if (strcmp(id, "sunflower_research") == 0) {
        p = &PROFILE_SUNFLOWER;
        lp = &LIGHT_RESEARCH_2OFF_3TO7_ABS18_5H;
        lp_id = "research_2off_3to7_abs18_5h";
    } else if (strcmp(id, "wheat_research") == 0) {
        p = &PROFILE_WHEAT;
        lp = &LIGHT_RESEARCH_2OFF_3TO7_ABS18_5H;
        lp_id = "research_2off_3to7_abs18_5h";
    } else if (strcmp(id, "pakbung_research") == 0) {
        p = &PROFILE_PAKBUNG;
        lp = &LIGHT_RESEARCH_2OFF_3TO7_ABS18_5H;
        lp_id = "research_2off_3to7_abs18_5h";
    }
    // --- คะน้า / โต้วเหมี่ยว ---
        ...
        p = &PROFILE_KALE;
        lp = &LIGHT_KALE_3OFF_4TO7_REL16;
        lp_id = "kale_3off_4to7_rel16";
    } else if (strcmp(id, "peashoot") == 0) {
        p = &PROFILE_PEASHOOT;
        lp = &LIGHT_PEASHOOT_3OFF_4TO9_REL14;
        lp_id = "peashoot_3off_4to9_rel14";
    }
    ...
    g_profile = *p;
    g_light_policy = *lp;
    strncpy(g_light_policy_id, lp_id, sizeof(g_light_policy_id) - 1);
    g_light_policy_id[sizeof(g_light_policy_id) - 1] = '\0';

    strncpy(g_profile_id, id, sizeof(g_profile_id) - 1);
    g_profile_id[sizeof(g_profile_id) - 1] = '\0';

    ESP_LOGI(TAG_GROW,
            "Applied crop profile: %s (soil %d–%d%%, fan T>=%.1fC/RH>=%.1f%%, light=%s)",
            g_profile_id,
...
/* ================= Tasks ================= */
// Grow light: per-crop policy (auto/manual unchanged)
static void grow_cycle_task(void *arg) {
    ...
    /* โหลด last_cmd_ms จาก NVS (กันซ้ำหลังรีบูต) */
    int64_t last_cmd_ms = 0;
    if (nvs_get_last_cmd_ms(&last_cmd_ms) == ESP_OK) {
        g_last_start_cycle_cmd_ms = last_cmd_ms;
        ESP_LOGI(TAG_GROW, "Loaded last_cmd_ms from NVS: %lld", (long long)last_cmd_ms);
    } else {
        g_last_start_cycle_cmd_ms = 0;
    }
    ...
    while (1) {
        ...
        if (elapsed_since(ctrl_poll_ms, 3000)) { // ทุก 3 วินาที
            ctrl_poll_ms = ms32();
            ...
            if (e == ESP_OK && cmd_ms > 0 && cmd_ms != g_last_start_cycle_cmd_ms) {
                /* ===== B: กันซ้ำแบบรีบูตไม่ซ้ำ ===== */
                g_last_start_cycle_cmd_ms = cmd_ms;
                nvs_set_last_cmd_ms(cmd_ms);

                /* รีเซ็ตเวลาเริ่มปลูก → Day 1 */
                time(&start_ts);
                nvs_set_start_ts(start_ts);

                /* รีเซ็ตตัวช่วย log/day */
                last_logged_grow_day = -1;   // ให้ log Day 1 ใหม่

                /* (กันสถานะน้ำค้างจากรอบเก่า) รีเซ็ตสถานะรอบรดน้ำ/ล็อก */
                pump_round_started_ms = 0;
                pump_doses_in_round   = 0;
                below_low_consec_cnt  = 0;
                pump_lock_until_ms    = 0;

                ESP_LOGW(TAG_GROW,
                        "Start cycle command accepted (ms=%lld) → reset Day 1",
                        (long long)cmd_ms);

                /* ===== A: เคลียร์ command ใน Firebase ให้หายไป ===== */
                esp_err_t clr = firebase_put(
                    "devices/" FIREBASE_DEVICE_ID "/control/start_cycle_ms",
                    "0"
                );
                if (clr != ESP_OK) {
                    ESP_LOGW(TAG_GROW, "Clear start_cycle_ms failed (will rely on NVS guard)");
                } else {
                    ESP_LOGI(TAG_GROW, "Cleared start_cycle_ms in Firebase (set to 0)");
                }
            }

            /* 2) crop_profile: เลือกชนิดพืชจากแอป */
            char new_profile[32] = {0};
        ...
        int total_days = g_light_policy.total_days;
        int day_display = (d + 1 <= total_days) ? (d + 1) : total_days;
        g_grow_day = day_display;

        if (day_display != last_logged_grow_day) {
            last_logged_grow_day = day_display;
            ESP_LOGI(TAG_GROW, "Grow Day %d/%d started (policy=%s, ts=%ld)",
                    day_display, total_days, g_light_policy_id, (long)now);
        }
        ...
        // รายงานสถานะไฟปลูกทุก 1 นาที + JSON
        if (elapsed_since(grow_status_ms, GROW_STATUS_LOG_MS)) {
            grow_status_ms = ms32();

            bool inwin_auto = false;
            if (g_light_policy.use_relative) {
                inwin_auto = is_in_light_window_relative_params(start_ts, now,
                                                                g_light_policy.off_days, g_light_policy.total_days,
                                                                g_light_policy.rel_offset_h,
                                                                g_light_policy.rel_offset_min,
                                                                g_light_policy.rel_duration_h);
                ...
                // ABS
                int day_idx = d + 1;
                if (day_idx > g_light_policy.off_days && day_idx <= g_light_policy.total_days) {
                    inwin_auto = is_in_light_window_absolute(now,
                                                            g_light_policy.abs_start_h,
                                                            g_light_policy.abs_start_m,
                                                            g_light_policy.abs_duration_h);
                }
            ...
            if (g_light_policy.use_relative) {
                ESP_LOGI(TAG_GROW,
                        "STATUS(1m): day=%d/%d LIGHT=%s window=%s (REL:+%dh/%dh, policy=%s, ctrl=%s)",
                        day_display, total_days,
                        g_light_on?"ON":"OFF",
                        inwin? "IN":"OUT",
                        g_light_policy.rel_offset_h, g_light_policy.rel_duration_h,
                        g_light_policy_id, ctrl_mode_str);
            } else {
                ESP_LOGI(TAG_GROW,
                        "STATUS(1m): day=%d/%d LIGHT=%s window=%s (ABS:%02d:%02d/%dh, policy=%s, ctrl=%s)",
                        day_display, total_days,
                        g_light_on?"ON":"OFF",
                        inwin? "IN":"OUT",
                        g_light_policy.abs_start_h, g_light_policy.abs_start_m,
            ...
            if (g_light_policy.use_relative) {
                ESP_LOGI("STATE",
                        ...
                        "\"policy\":\"%s\",\"ctrl_mode\":\"%s\"}",
                        (long long)tsms, iso, day_display,
                        g_light_on? "true":"false",
                        inwin? "true":"false",
                        g_light_policy.rel_offset_h, g_light_policy.rel_duration_h,
                        g_light_policy_id, ctrl_mode_json);
            } else {
                ESP_LOGI("STATE",
                        "{\"ts\":%lld,\"iso\":\"%s\",\"mod\":\"grow\",\"day\":%d,"
                        "\"light\":%s,\"in_window\":%s,\"mode\":\"ABS\",\"start_h\":%d,\"start_m\":%d,\"dur_h\":%d,"
                        "\"policy\":\"%s\",\"ctrl_mode\":\"%s\"}",
                        (long long)tsms, iso, day_display,
                        g_light_on? "true":"false",
                        inwin? "true":"false",
                        g_light_policy.abs_start_h, g_light_policy.abs_start_m, g_light_policy.abs_duration_h,
                        g_light_policy_id, ctrl_mode_json);
            }
        ...
        if (d >= 7) { // คงเดิมตามคอมไพล์ออปชันเดิม
...
static void soil_pump_task(void *arg)
{
    ...
    gpio_reset_pin(RELAY_SOLENOID_GPIO);
    gpio_set_direction(RELAY_SOLENOID_GPIO, GPIO_MODE_OUTPUT);

    /* ให้เริ่มต้นเป็น OFF แน่นอน */
    relay2ch_write(RELAY_PUMP_GPIO, false);
    relay2ch_write(RELAY_SOLENOID_GPIO, false);
    pumpOn = false;
    solenoidOn = false;
