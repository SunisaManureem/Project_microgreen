// ===== microgreen_control.c (Wi-Fi start→scan fix, BSSID lock, backoff, WDT-safe, RAW-calibrated ADC, pulse irrigation, multi-factor fan) =====
#include <stdio.h>
#include <string.h>
#include <time.h>
...
/* Grow-light minute policy (test mode) */
...
/* ===== Calibrated RAW endpoints (from your rig) =====
   Use "real tray wet" as wet point (not submerge in water). */
static int rawDry = 2688;    // dry/air (your measured)
static int rawWet = 1565;    // wet in tray after soak (your measured)

/* Moisture hysteresis (%) — field tuned to 65–78 to reduce overwatering */
...
/* NEW: post-unlock grace window (allow dose even if sensor looks "stuck") */
static uint32_t pump_post_unlock_grace_until_ms = 0;
static inline bool in_post_unlock_grace(void){
    if (pump_post_unlock_grace_until_ms == 0) return false;
    return (uint32_t)(pump_post_unlock_grace_until_ms - ms32()) > 0;
}
...
static void set_pump(bool on)
{
    ...
    if (on) {
        ...
        /* lockout + post-unlock grace (20s after lock expires) */
        pump_lock_until_ms = pump_last_change_ms + PUMP_LOCKOUT_MS;
        pump_post_unlock_grace_until_ms = pump_lock_until_ms + 20000;  // <<< grace
    }
...
// Soil → Pump (pulse irrigation) — RAW calibrated + fail-safe (tolerant stuck detector + grace)
static void soil_pump_task(void *arg)
{
    ...
    pump_post_unlock_grace_until_ms = 0;
    ...
    while (1) {
        ...
        // treat only "hard invalid" as blocker for starting a dose
        ...
        // accumulate confirm only when really below low (ignore suspect state)
        ...
        /* Allow opening if:
           - not ON, not hard-invalid, lockout expired
           - doses in round < max, min OFF elapsed, AND confirmed low moisture
           - AND (sensor not suspect) OR (we are in 20s grace right after lockout)
        */
        bool can_try_open =
            (!pumpOn) &&
            !hard_invalid &&
        ...
        // While ON: if sensor looks suspect and we've satisfied minimum ON → turn OFF (safety)
        if (suspect) {
