...
static void soil_pump_task(void *arg)
{
    ...
    while (1) {
        ...
        // NEW: ไม่รีเซ็ตตัวนับเฉพาะเพราะ suspect; รีเซ็ตก็ต่อเมื่อความชื้นไม่ต่ำกว่า TH ต่ำ
        ...
        // NEW: อนุญาตเริ่มโดสแม้ suspect==true (ค่านิ่ง) แต่ต้องไม่ hard_invalid
        bool can_try_open =
            (!pumpOn) &&
            !hard_invalid &&                           // block only truly invalid RAW
