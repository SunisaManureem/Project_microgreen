...
// REAL-DAY grow light (REL window: start+6h → 14h ON for Day4–7)
...
static bool wifi_scan_and_lock_target(const char* target_ssid, uint8_t out_bssid[6], uint8_t *out_chan){
    ...
    if (best>=0){
        ...
        *out_chan = recs[best].primary;
        ...
        ESP_LOGW(TAG_NET, "Scan: SSID '%s' not found in records");
    }
    free(recs);
    return ok;
}
...
static void grow_cycle_task(void *arg) {
    ...
    while (1) {
        ...
        if (gpio_get_level(RESET_BTN_PIN) == 0) {
            ...
            // ≥8s → clear Wi-Fi creds+lock + start_ts → restart
            if (elapsed_since(btn_ms, WIFI_CLEAR_HOLD_MS)) {
                clear_wifi_creds();
                nvs_wifi_clear_lock();
                nvs_clear_start_ts();
                ESP_LOGW(TAG_GROW, "Button(8s): clear Wi-Fi lock + start_ts, restarting…");
        ...
        // ===== เช็คคำสั่งจาก Firebase: start new cycle (จากแอป) =====
...
static void soil_pump_task(void *arg)
{
    ...
    while (1) {
        ...
        /* ===== NEW: ตัดปั๊มทันทีเมื่อความชื้นถึง HIGH (≥78%) — ไม่รอ 15s ===== */
        if (pumpOn && pct >= SOIL_TH_HIGH) {
            set_pump(false);  // ยังมี short lock 3 นาทีตามเดิม
            ESP_LOGI(TAG_SOIL, "Cutoff: moisture ≥ %d%% → stop now", SOIL_TH_HIGH);
        }

        // ปิดเมื่อครบโดส 6 วิ (คงเดิม)
