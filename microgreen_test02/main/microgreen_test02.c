// ===== microgreen_control.c (Wi-Fi start→scan fix, BSSID lock, backoff, WDT-safe, calibrated ADC, pulse irrigation, multi-factor fan) =====
#include <stdio.h>
#include <string.h>
#include <time.h>
...
#include "driver/adc.h"          // legacy ADC ok for simple oneshot
#include "esp_rom_sys.h"
#include "esp_adc_cal.h"         // legacy characterize
...
/* Calibrated endpoints (mV after mapping) */
static uint32_t mV_Dry = 2100;   // ~from rawDry 2688 @ 12dB
static uint32_t mV_Wet = 1200;   // ~from rawWet 1565 @ 12dB
...
static const uint32_t FAN_MIN_ON_MS       = 30*1000;
static const uint32_t FAN_MIN_OFF_MS      = 30*1000;
...
static inline bool not_yet(uint32_t until_ms){
    ...
    return (uint32_t)(until_ms - ms32()) > 0;
}
...
/* ================= ADC calibration (legacy) ================= */
static esp_adc_cal_characteristics_t adc_chars;
static bool adc_cal_inited = false;

static void adc_cal_init(void){
    adc1_config_width(ADC_WIDTH_BIT_12);
    adc1_config_channel_atten(SOIL_ADC_CH, SOIL_ADC_ATTEN);
    adc_cal_inited = esp_adc_cal_characterize(ADC_UNIT_1, SOIL_ADC_ATTEN, ADC_WIDTH_BIT_12,
                                              1100, &adc_chars) != ESP_ADC_CAL_VAL_NOT_SUPPORTED;
    if (adc_cal_inited) {
        ESP_LOGI(TAG_SOIL, "ADC calibration enabled (Vref=%dmV)", adc_chars.vref);
    } else {
        ESP_LOGW(TAG_SOIL, "ADC calibration not supported; fallback to raw mapping");
    }
}

static int adc_read_raw(void){
    return adc1_get_raw(SOIL_ADC_CH);
}

static uint32_t raw_to_mV(int raw){
    if (!adc_cal_inited) {
        return (uint32_t)((raw * 2500) / 4095); // conservative fallback
    }
    return esp_adc_cal_raw_to_voltage(raw, &adc_chars);
}

/* Fail-sensor detection */
static int      last_raw = -1;
static int      same_raw_cnt = 0;
static int      stuck_raw_threshold = 20;
static const int RAW_MIN_VALID = 50;
static const int RAW_MAX_VALID = 4045;

static bool soil_sensor_suspect(int raw){
    if (raw == last_raw) same_raw_cnt++; else { same_raw_cnt = 0; last_raw = raw; }
    if (raw <= RAW_MIN_VALID || raw >= RAW_MAX_VALID) return true;
    if (same_raw_cnt >= stuck_raw_threshold) return true;
    return false;
}

/* mV -> % moisture (wet = lower mV) */
static int soil_mV_to_pct(uint32_t mV){
    long num = (long)((int32_t)mV - (int32_t)mV_Dry) * 100L;
    long den = (long)((int32_t)mV_Wet - (int32_t)mV_Dry);
    if (den == 0) return 0;
    long pct = num / den;
    if (pct < 0) pct = 0;
    if (pct > 100) pct = 100;
...
static void wifi_init_sta_once(void)
{
    ...
    // *** FIX: use IP_EVENT_* (not ESP_EVENT_*) ***
    ESP_ERROR_CHECK(esp_event_handler_instance_register(
...
#ifdef CONFIG_WPA3_SAE_PWE_H2E  // guard if symbol exists
...
/* ================= Tasks ================= */
// Grow light: 7-min cycle (1–3 OFF, 4–7 ON)
...
// Soil → Pump (pulse irrigation) + ADC calibration + fail-safe
static void soil_pump_task(void *arg)
{
    ...
    while (1) {
        ...
        bool can_try_open =
            ...
            elapsed_since(pump_last_change_ms, PUMP_MIN_OFF_MS) &&
        ...
        if (suspect) {
            ...
            if (elapsed_since(last_suspect_log_ms, 10000)) {
                ...
                ESP_LOGW(TAG_SOIL, "Soil sensor suspect: RAW=%d mV=%u same_cnt=%d", raw, mV, same_raw_cnt);
            }
        }
        ...
        if (elapsed_since(hb_ms, HEARTBEAT_MS)) {
            ...
            if (last_soil_pct_logged < 0 || (abs(pct - last_soil_pct_logged) >= 3)) {
                ...
                ESP_LOGI(TAG_SOIL, "Moisture=%d%% RAW=%d mV=%u Pump=%s dose#=%d lock=%s",
                         pct, raw, mV, pumpOn?"ON":"OFF", pump_doses_in_round, not_yet(pump_lock_until_ms)?"Y":"N");
            } else {
