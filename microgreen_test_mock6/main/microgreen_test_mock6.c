...
// REAL-DAY grow light with 7-day counter + 14h window
...
/* ================= Grow-light policy (REAL DAYS in Thai time) =================
   Day 1–3: OFF
   Day 4–7: ON (14 ชั่วโมง/วัน ตามหน้าต่างเวลา)
...
#define LIGHT_ON_START_HOUR     6   // 06:00
#define LIGHT_ON_START_MIN      0
#define LIGHT_ON_DURATION_H     14  // 14 ชั่วโมง

/* ==================== TEST MODE (FOR EASY DEMO) ====================
   1 นาที = 1 วัน:
   นาที 1–3 ปิด, นาที 4–7 เปิดเฉพาะวินาที 6–19, นาที >=8 ปิด
*/
#define GROW_TEST_MINUTE_DAY       1        // 1=เปิดโหมดทดสอบ, 0=โหมดจริง
#define TEST_LIGHT_SEC_START       6
#define TEST_LIGHT_SEC_END         19
/* ================================================================== */
...
#define SOIL_SAMPLES_MEDIAN     21

/* ===== Calibrated RAW endpoints (จากถาดจริง) =====
   ดินแห้ง≈2100, ชุ่มพร้อมปลูก≈1500 */
static int rawDry = 2100;
static int rawWet = 1500;

/* Moisture hysteresis (%) */
static int SOIL_TH_LOW  = 65;    // ≤65% เริ่มจ่าย
static int SOIL_TH_HIGH = 78;    // ≥78% หยุดจ่าย

/* Pump timings */
// static const uint32_t PUMP_MIN_ON_MS      = 15*1000; // (ลบออก: ปิดทันที)
...
/* ===== Logging cadence (อ่านง่าย + ส่งขึ้นคลาวด์ง่าย) ===== */
static const uint32_t HEARTBEAT_SOIL_MS   = 2000;    // soil snapshot ทุก 2s
static const uint32_t HEARTBEAT_DHT_MS    = 2000;    // env snapshot ทุก 2s
static const uint32_t GROW_STATUS_LOG_MS  = 60000;   // grow status ทุก 1 นาที
...
static const char *TAG_STATE = "STATE";     // ❗ JSON snapshot สำหรับเอาไปขึ้น Firebase ได้ทันที
...
static inline bool not_yet(uint32_t until_ms){
    ...
    return (int32_t)(until_ms - ms32()) > 0; // signed delta
}

/* ===== Time helpers (ISO8601 & epoch ms) ===== */
static inline uint64_t epoch_ms(void){
    struct timeval tv; gettimeofday(&tv, NULL);
    return (uint64_t)tv.tv_sec*1000ULL + (uint64_t)(tv.tv_usec/1000ULL);
}
static void iso8601_local(char* out, size_t n){
    time_t now = time(NULL);
    struct tm ti; localtime_r(&now, &ti);
    // %z → +0700; แทรก ':' ให้เป็น +07:00 เพื่ออ่านง่าย
    char buf[40]; strftime(buf, sizeof(buf), "%Y-%m-%dT%H:%M:%S%z", &ti);
    size_t L = strlen(buf);
    if (L >= 5 && n > 0){
        // แทรก ':' ก่อน 2 ตัวท้าย
        // ex: 2025-11-01T10:20:30+0700 -> +07:00
        memmove(buf + L + 1, buf + L - 2, 3); // "00\0"
        buf[L - 2] = ':';
    }
    strncpy(out, buf, n-1);
    out[n-1] = '\0';
...
static const int RAW_TOLERANCE       = 10;
static const int RAW_STUCK_THRESHOLD = 3600;
...
static uint32_t s_next_reconn_ms = 0;
static uint32_t s_backoff_ms     = 1000;
static int      s_disc_streak    = 0;
...
/* scan for target SSID and lock best BSSID+channel (call AFTER start) */
static bool wifi_scan_and_lock_target(const char* target_ssid, uint8_t out_bssid[6], uint8_t *out_chan){
    ...
    for (int i=0;i<n;i++){
        if (strncmp((const char*)recs[i].ssid, target_ssid, 32)==0){
            if (recs[i].rssi > best_rssi){ best_rssi = recs[i].rssi; best = i; }
        }
    }
    bool ok=false;
    if (best>=0){
...
static int units_since_start(time_t start_ts, time_t now) {
    ...
    if (now <= start_ts) return 0;
    double diff_sec = difftime(now, start_ts);
    return (int)(diff_sec / 60.0);  // minute
#else
    return days_since(start_ts, now);  // day
#endif
}
...
static void apply_light_policy_day(int d, time_t now_ts) {
    if (d < OFF_DAYS) {
        grow_off();
        ESP_LOGI(TAG_GROW, "Day %d: LIGHT=OFF (seedling dark)", d + 1);
        ...
        if (is_in_light_window(now_ts)) {
            ...
            grow_off();
            ESP_LOGI(TAG_GROW, "Day %d: LIGHT=OFF (outside window)", d + 1);
        }
    } else {
        grow_off();
        ESP_LOGI(TAG_GROW, "Day %d+: LIGHT=OFF (beyond 7)", d + 1);
    }
}
#endif
...
static void apply_light_policy_minute(int m, time_t now_ts) {
    ...
    if (m < OFF_DAYS) {
        ...
        want_on = is_in_light_window_test(now_ts) ? 1 : 0;
    } else {
        want_on = 0;
    }
    if (last_light != want_on) {
        last_light = want_on;
        if (want_on) { grow_on();  ESP_LOGI(TAG_GROW, "Minute %d: LIGHT=ON",  m + 1); }
        else         { grow_off(); ESP_LOGI(TAG_GROW, "Minute %d: LIGHT=OFF", m + 1); }
    }
}
#endif
...
/* ================= Tasks ================= */
// Grow light: REAL-DAY / TEST-MINUTE
static void grow_cycle_task(void *arg) {
    ...
    static uint32_t grow_status_ms = 0;   // log สถานะไฟทุก 1 นาที
    grow_status_ms = ms32() - GROW_STATUS_LOG_MS; // พิมพ์ครั้งแรกทันที
    ...
    while (1) {
        ...
        /* ---- สถานะไฟปลูก: มนุษย์อ่าน + JSON พร้อมดันขึ้น Firebase ---- */
        if (elapsed_since(grow_status_ms, GROW_STATUS_LOG_MS)) {
            grow_status_ms = ms32();

            char iso[48]; iso8601_local(iso, sizeof(iso));
#if GROW_TEST_MINUTE_DAY
            bool inwin = is_in_light_window_test(now);
            ESP_LOGI(TAG_GROW, "STATUS(min): minute=%d LIGHT=%s window=%s",
                     display, g_light_on?"ON":"OFF", inwin? "IN-SEC(6–19)" : "OUT-SEC");

            // JSON snapshot (grow)
            ESP_LOGI(TAG_STATE,
                "{\"ts\":%llu,\"iso\":\"%s\",\"mod\":\"grow\",\"mode\":\"test\","
                "\"minute\":%d,\"light\":%s,\"in_window\":%s}",
                (unsigned long long)epoch_ms(), iso, display,
                g_light_on?"true":"false", inwin?"true":"false");
#else
            bool inwin = is_in_light_window(now);
            ESP_LOGI(TAG_GROW, "STATUS(min): day=%d LIGHT=%s window=%s (06:00–20:00,14h)",
                     display, g_light_on?"ON":"OFF", inwin? "IN" : "OUT");

            // JSON snapshot (grow)
            ESP_LOGI(TAG_STATE,
                "{\"ts\":%llu,\"iso\":\"%s\",\"mod\":\"grow\",\"mode\":\"real\","
                "\"day\":%d,\"light\":%s,\"in_window\":%s}",
                (unsigned long long)epoch_ms(), iso, display,
                g_light_on?"true":"false", inwin?"true":"false");
#endif
        }
        ...
        if (elapsed_since(hb_ms, 5000)) { // debug heartbeat
            hb_ms = ms32();
            ESP_LOGD(TAG_GROW, "HB: u=%d display=%d light=%d", u, display, g_light_on?1:0);
        }

        esp_task_wdt_reset();
#if GROW_TEST_MINUTE_DAY
        vTaskDelay(pdMS_TO_TICKS(200));   // precise sec 6–19
#else
        vTaskDelay(pdMS_TO_TICKS(1000));
#endif
    }
...
static void soil_pump_task(void *arg) {
    ...
    while (1) {
        ...
        // == ปิดทันทีตามที่ต้องการ ==
        if (pumpOn && pct >= SOIL_TH_HIGH) {
            set_pump(false);
        }

        if (suspect) {
            static uint32_t last_suspect_log_ms = 0;
            // == ปิดทันทีถ้า sensor น่าสงสัย ==
            if (pumpOn) {
                set_pump(false);
            }
        ...
        /* ---- Snapshot มนุษย์อ่าน + JSON สำหรับคลาวด์ ---- */
        if (elapsed_since(hb_ms, HEARTBEAT_SOIL_MS)) {
            hb_ms = now;

            // human-friendly
            ESP_LOGI(TAG_SOIL, "STATUS(2s): Moisture=%d%% RAW=%d Pump=%s dose#=%d lock=%s",
                     pct, raw, pumpOn?"ON":"OFF", pump_doses_in_round, not_yet(pump_lock_until_ms)?"Y":"N");

            // JSON snapshot (soil)
            char iso[48]; iso8601_local(iso, sizeof(iso));
            ESP_LOGI(TAG_STATE,
                "{\"ts\":%llu,\"iso\":\"%s\",\"mod\":\"soil\","
                "\"moisture\":%d,\"raw\":%d,"
                "\"pump\":%s,\"dose\":%d,\"lock\":%s}",
                (unsigned long long)epoch_ms(), iso,
                pct, raw,
                pumpOn?"true":"false",
                pump_doses_in_round,
                not_yet(pump_lock_until_ms)?"true":"false");
        }
...
static void dht22_fan_task(void *arg) {
    ...
    while (1) {
        ...
        if (!ok) {
            ...
            uint32_t now = ms32();
            bool can_on  = (!fanOn) && elapsed_since(fan_last_change_ms, 10000); // min off 10s
            bool can_off = ( fanOn) && elapsed_since(fan_last_change_ms, 10000); // min on 10s
