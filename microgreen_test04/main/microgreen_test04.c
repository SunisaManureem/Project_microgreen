...
/* Fail-sensor detection (on RAW)
   UPDATED: add tolerance ±5 and extend stuck threshold to ~5min (300 reads @1Hz) */
static int      last_raw = -1;
static int      same_raw_cnt = 0;
static const int RAW_MIN_VALID = 50;
static const int RAW_MAX_VALID = 4045;

/* New tuning knobs (as discussed) */
static const int RAW_TOLERANCE         = 5;     // treat changes within ±5 as "no change"
static const int RAW_STUCK_THRESHOLD   = 300;   // 300 consecutive "no-change" ≈ 5 minutes
