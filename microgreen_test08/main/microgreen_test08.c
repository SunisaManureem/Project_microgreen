// ===== microgreen_control.c (Option B: allow start even if suspect; keep safety-off while ON; add DECIDE logs) =====
#include <stdio.h>
#include <string.h>
#include <time.h>
...
// Soil → Pump (pulse irrigation) — Option B applied
static void soil_pump_task(void *arg)
{
    ...
    while (1) {
        ...
        // OPTION B: treat only "hard invalid" as blocker for starting a dose
        ...
        // ไม่รีเซ็ตตัวนับเพียงเพราะ suspect; รีเซ็ตเมื่อไม่ต่ำกว่า TH หรือ RAW hard-invalid เท่านั้น
        ...
        // OPTION B: อนุญาตเริ่มโดสแม้ suspect==true (ตราบใดไม่ hard-invalid)
        bool can_try_open =
            (!pumpOn) &&
            !hard_invalid &&
        ...
        // หยุดเมื่อชื้นถึง HIGH และผ่าน min-on แล้ว
        if (pumpOn && pct >= SOIL_TH_HIGH && elapsed_since(pump_last_change_ms, PUMP_MIN_ON_MS)) {
            set_pump(false);
        }

        // Safety ขณะ ON: ถ้าเซ็นเซอร์ถูกสงสัย → ปิดหลังพ้น min-on
        if (suspect) {
        ...
        // DECIDE log (ทุก 2 วินาที) — บอกเหตุผลที่ยังไม่ ON
        if (elapsed_since(dbg_ms, 2000)) {
            dbg_ms = now;
            ESP_LOGI(TAG_SOIL,
                "DECIDE: lock=%d hard_invalid=%d suspect=%d below_cnt=%d off_gap_ok=%d doses=%d",
                not_yet(pump_lock_until_ms),
                hard_invalid,
                suspect,
                below_low_consec_cnt,
                elapsed_since(pump_last_change_ms, PUMP_MIN_OFF_MS),
                pump_doses_in_round
            );
        }
